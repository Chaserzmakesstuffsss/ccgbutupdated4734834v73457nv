...
function formatEveryThirdPower(notations)
{
	return function (val)
	{
		...
		if (!isFinite(val)) return 'Infinity...Jk';
		if (val>=1000000)
		{
			val/=1000;
			...
			if (base>=notations.length) {return 'Infinity...Jk';} else {notationValue=notations[base];}
		}
...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		TopBarOffset=32;
		if (!App) Game.wrapper.classList.add('offWeb');
		else {Game.wrapper.classList.add('onWeb');TopBarOffset=0;}
	...
	Game.Logic=function()
	{
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.handmadeCookies>=3) Game.Unlock('Jukebox')
